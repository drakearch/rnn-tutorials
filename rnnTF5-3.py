...
seq_len = [truncated_backprop_length for i in range(batch_size)]

batchX_placeholder = tf.placeholder(tf.float32, [batch_size, truncated_backprop_length])
batchY_placeholder = tf.placeholder(tf.int32, [batch_size, truncated_backprop_length])

W2 = tf.Variable(np.random.rand(state_size*2, num_classes),dtype=tf.float32)
...
states_series, current_state = tf.nn.bidirectional_dynamic_rnn(cell, cell, tf.expand_dims(batchX_placeholder, -1), sequence_length=seq_len, dtype=tf.float32)
states_series = tf.concat([states_series[0], states_series[1]], 2)
